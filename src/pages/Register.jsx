...
export default function Register() {
  ...
  const [form, setForm]           = useState({ nome: '', sobrenome: '', email: '', telefone: '', senha: '', confirma: '', renda: '', objetivo: '' })
  ...
  async function handleSubmit() {
    ...
    try {
      const res = await register({
        ...
        telefone: form.telefone, perfil: 'USER', plano: selectedPlan,
      })
  ...
  if (done) return (
    <div style={{ minHeight: '100vh', background: 'var(--bg)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexDirection: 'column', gap: 20 }}>
      <div style={{ width: 80, height: 80, borderRadius: '50%', background: 'rgba(74,240,196,0.1)', border: '1px solid rgba(74,240,196,0.3)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 36 }}>✅</div>
      <h2 style={{ fontFamily: 'Syne, sans-serif', fontSize: 28, fontWeight: 800 }}>Conta criada com sucesso!</h2>
      <p style={{ color: 'var(--muted)', fontSize: 15 }}>Redirecionando para o dashboard...</p>
    </div>
  )
  ...
  return (
    <div style={{ minHeight: '100vh', background: 'var(--bg)', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '40px 20px', position: 'relative', overflow: 'hidden' }}>
      ...
      <div style={{ position: 'fixed', inset: 0, pointerEvents: 'none', zIndex: 0 }}>
        <div style={{ position: 'absolute', width: 600, height: 600, borderRadius: '50%', background: 'radial-gradient(circle, rgba(26,75,204,0.25) 0%, transparent 70%)', top: -200, left: -100, filter: 'blur(120px)' }} />
        <div style={{ position: 'absolute', width: 400, height: 400, borderRadius: '50%', background: 'radial-gradient(circle, rgba(74,240,196,0.08) 0%, transparent 70%)', bottom: -100, right: -80, filter: 'blur(100px)' }} />
      </div>

      <div style={{ width: '100%', maxWidth: step === 3 ? 1080 : 500, position: 'relative', zIndex: 2 }}>

        {/* Logo */}
        <div style={{ fontFamily: 'Syne, sans-serif', fontSize: 22, fontWeight: 800, textAlign: 'center', marginBottom: 36, letterSpacing: 3, color: 'var(--white)' }}>
          MONETO
        </div>

        {/* Header */}
        <div style={{ textAlign: 'center', marginBottom: 32 }}>
          <h1 style={{ fontFamily: 'Syne, sans-serif', fontSize: 26, fontWeight: 800, letterSpacing: -0.5, marginBottom: 8 }}>
            ...
            return (
              <div key={label} style={{ display: 'flex', alignItems: 'flex-start' }}>
                <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 6 }}>
                  ...
                  <span style={{ fontSize: 11, color: isActive ? 'var(--white)' : isDone ? '#4af0c4' : 'var(--muted)', fontWeight: isActive ? 600 : 400, whiteSpace: 'nowrap' }}>
                    {label}
                  </span>
                </div>
                {i < stepLabels.length - 1 && (
                  <div style={{ width: 60, height: 1.5, background: step > n ? 'rgba(74,240,196,0.4)' : 'rgba(255,255,255,0.06)', margin: '16px 8px 0', transition: 'background 0.3s ease', flexShrink: 0 }} />
                )}
              </div>
            )
          })}
        ...
        {step === 1 && (
          <div style={styles.card}>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14, marginBottom: 0 }}>
              <InputField label="Nome" placeholder="João" value={form.nome} onChange={v => handle('nome', v)} error={errors.nome} />
              <InputField label="Sobrenome" placeholder="Silva" value={form.sobrenome} onChange={v => handle('sobrenome', v)} error={errors.sobrenome} />
            </div>
            <InputField label="E-mail" type="email" placeholder="joao@exemplo.com" value={form.email} onChange={v => handle('email', v)} error={errors.email} />
            <InputField label="WhatsApp" placeholder="+55 (11) 99999-9999" value={form.telefone} onChange={v => handle('telefone', v)} error={errors.telefone} />
            ...
            <div style={styles.fieldWrap}>
              ...
              <div style={{ position: 'relative' }}>
                <input
                  ...
                  style={{ ...styles.input, paddingRight: 48, borderColor: errors.senha ? 'rgba(240,106,106,0.5)' : form.senha && strength.pct === 100 ? 'rgba(74,240,196,0.3)' : 'var(--border)' }}
                />
                <button
                  type="button"
                ...
                <div style={{ marginTop: 10 }}>
                  <div style={{ display: 'flex', gap: 4, marginBottom: 6 }}>
                    {[25, 50, 75, 100].map(threshold => (
                      <div key={threshold} style={{ flex: 1, height: 3, borderRadius: 2, background: strength.pct >= threshold ? strength.color : 'rgba(255,255,255,0.06)', transition: 'background 0.3s ease' }} />
                    ))}
                  </div>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    <span style={{ fontSize: 12, color: strength.color, fontWeight: 600 }}>{strength.text}</span>
                    <span style={{ fontSize: 11, color: 'var(--muted)' }}>{strength.label}</span>
                  </div>
                </div>
              )}
            </div>
            ...
            <div style={styles.fieldWrap}>
              ...
              <div style={{ position: 'relative' }}>
                <input
                  ...
                  style={{ ...styles.input, paddingRight: 48, borderColor: errors.confirma ? 'rgba(240,106,106,0.5)' : form.confirma && form.confirma === form.senha ? 'rgba(74,240,196,0.3)' : 'var(--border)' }}
                />
                <button
                  type="button"
                ...
                {form.confirma && form.confirma === form.senha && (
                  <div style={{ position: 'absolute', right: 48, top: '50%', transform: 'translateY(-50%)', color: '#4af0c4' }}>
                    <CheckIcon />
                  </div>
                )}
              </div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 12, marginBottom: 24 }}>
              {plans.map(p => (
                ...
                >
                  ...
                  <div style={{ fontFamily: 'Syne, sans-serif', fontSize: 13, fontWeight: 800, marginBottom: 8, letterSpacing: 0.3 }}>{p.name}</div>
                  <div style={{ marginBottom: 8 }}>
                    <span style={{ fontFamily: 'Syne, sans-serif', fontSize: 24, fontWeight: 800, color: p.color, letterSpacing: -1 }}>{p.price}</span>
                    <span style={{ fontSize: 12, color: 'var(--muted)', marginLeft: 4 }}>{p.period}</span>
                  </div>
                  <p style={{ fontSize: 12, color: 'var(--muted)', lineHeight: 1.5, marginBottom: 14 }}>{p.desc}</p>

                  <div style={{ display: 'flex', flexDirection: 'column', gap: 7 }}>
                    {p.features.map(f => (
                      <div key={f} style={{ display: 'flex', gap: 7, alignItems: 'flex-start' }}>
                        <span style={{ color: p.color, flexShrink: 0, marginTop: 1 }}><CheckIcon /></span>
                        <span style={{ fontSize: 11, color: 'rgba(240,246,255,0.7)', lineHeight: 1.4 }}>{f}</span>
                      </div>
                    ))}
                    {p.limits.map(f => (
                      <div key={f} style={{ display: 'flex', gap: 7, alignItems: 'flex-start' }}>
                        <span style={{ color: 'rgba(255,255,255,0.15)', flexShrink: 0, marginTop: 1 }}><MinusIcon /></span>
                        <span style={{ fontSize: 11, color: 'rgba(255,255,255,0.2)', lineHeight: 1.4 }}>{f}</span>
                      </div>
                    ))}
                  </div>
                </div>
        ...
        {step === 4 && (
          <div style={styles.card}>
            ...
            <div style={{ marginBottom: 20, padding: 14, background: `${selectedPlanData?.color}10`, border: `1px solid ${selectedPlanData?.border}`, borderRadius: 12, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
              <div>
                <div style={{ fontSize: 12, color: 'var(--muted)', marginBottom: 2 }}>Plano selecionado</div>
                <div style={{ fontFamily: 'Syne, sans-serif', fontSize: 15, fontWeight: 700, color: selectedPlanData?.color }}>{selectedPlanData?.name}</div>
              </div>
              <div style={{ fontFamily: 'Syne, sans-serif', fontSize: 16, fontWeight: 800, color: selectedPlanData?.color }}>{selectedPlanData?.price}<span style={{ fontSize: 11, fontWeight: 400, color: 'var(--muted)' }}>{selectedPlanData?.period}</span></div>
            </div>
  ...
  return (
    <div style={styles.fieldWrap}>
      ...
      <input
        ...
        style={{ ...styles.input, borderColor: error ? 'rgba(240,106,106,0.5)' : 'var(--border)' }}
      />
...
const styles = {
  ...
  label: {
    ...
    letterSpacing: 0.3, color: 'rgba(240,246,255,0.6)',
    marginBottom: 8,
  },
  input: {
  ...
  errorMsg: { fontSize: 12, color: '#f06a6a', marginTop: 6, display: 'block' },
  btnPrimary: {
  ...
  btnGhost: {
    ...
    transition: 'all 0.2s', marginTop: 4, whiteSpace: 'nowrap',
  },
}